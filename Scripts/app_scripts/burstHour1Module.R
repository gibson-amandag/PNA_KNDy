### Comparing Burst Windows - Hour 1

# https://shiny.rstudio.com/articles/modules.html

burstHour1UI <- function(
  id
){
  ns <- NS(id)
  tagList(
    
    h2("Burst Parameters - First 60 min of Recording"),
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      burstsPerHour_plots <- burstParamsServer(
        ...
        var_to_plot = expr(BurstsPerHour_hour1), #as expr()
        var_to_plot_BW1 = expr(BurstsPerHour_hour1_BW1), #as expr()
        var_to_plot_BW2 = expr(BurstsPerHour_hour1_BW2), #as expr()
      ...
      mbd_plots <- burstParamsServer(
        "mbd",
        df = KNDyDATA_filtered, #use a reactive df, but don't include ()
        var_to_plot = expr(Mbd_hour1), #as expr()
        var_to_plot_BW1 = expr(Mbd_hour1_BW1), #as expr()
        var_to_plot_BW2 = expr(Mbd_hour1_BW2), #as expr()
        treatment = selectData$treatment, #reactive value, but don't include ()
        grouping_var = selectData$grouping_var, #reactive value, but don't include ()
        expand_to_zero = reactive({ input$expand_to_zero }), #TRUE/FALSE
        dotLayer = reactive({ input$dot_plot }), #TRUE/FALSE
      ...
      spb_plots <- burstParamsServer(
        "spb",
        df = KNDyDATA_filtered, #use a reactive df, but don't include ()
        var_to_plot = expr(SpikesPerBurst_hour1), #as expr()
        var_to_plot_BW1 = expr(SpikesPerBurst_hour1_BW1), #as expr()
        var_to_plot_BW2 = expr(SpikesPerBurst_hour1_BW2), #as expr()
        treatment = selectData$treatment, #reactive value, but don't include ()
        grouping_var = selectData$grouping_var, #reactive value, but don't include ()
        expand_to_zero = reactive({ input$expand_to_zero }), #TRUE/FALSE
        dotLayer = reactive({ input$dot_plot }), #TRUE/FALSE
      ...
      burstFreq_plots <- burstParamsServer(
        "burstFreq",
        df = KNDyDATA_filtered, #use a reactive df, but don't include ()
        var_to_plot = expr(BurstFreq_hour1), #as expr()
        var_to_plot_BW1 = expr(BurstFreq_hour1_BW1), #as expr()
        var_to_plot_BW2 = expr(BurstFreq_hour1_BW2), #as expr()
        treatment = selectData$treatment, #reactive value, but don't include ()
        grouping_var = selectData$grouping_var, #reactive value, but don't include ()
        expand_to_zero = reactive({ input$expand_to_zero }), #TRUE/FALSE
        dotLayer = reactive({ input$dot_plot }), #TRUE/FALSE
      ...
      ssn_plots <- burstParamsServer(
        "ssn",
        df = KNDyDATA_filtered, #use a reactive df, but don't include ()
        var_to_plot = expr(SingleSpikeNum_hour1), #as expr()
        var_to_plot_BW1 = expr(SingleSpikeNum_hour1_BW1), #as expr()
        var_to_plot_BW2 = expr(SingleSpikeNum_hour1_BW2), #as expr()
        treatment = selectData$treatment, #reactive value, but don't include ()
        grouping_var = selectData$grouping_var, #reactive value, but don't include ()
        expand_to_zero = reactive({ input$expand_to_zero }), #TRUE/FALSE
        dotLayer = reactive({ input$dot_plot }), #TRUE/FALSE
      ...
      ssf_plots <- burstParamsServer(
        "ssf",
        df = KNDyDATA_filtered, #use a reactive df, but don't include ()
        var_to_plot = expr(SingleSpikeFreq_hour1), #as expr()
        var_to_plot_BW1 = expr(SingleSpikeFreq_hour1_BW1), #as expr()
        var_to_plot_BW2 = expr(SingleSpikeFreq_hour1_BW2), #as expr()
        treatment = selectData$treatment, #reactive value, but don't include ()
        grouping_var = selectData$grouping_var, #reactive value, but don't include ()
        expand_to_zero = reactive({ input$expand_to_zero }), #TRUE/FALSE
        dotLayer = reactive({ input$dot_plot }), #TRUE/FALSE
      ...
      totalFreq_plots <- burstParamsServer(
        "totalFreq",
        df = KNDyDATA_filtered, #use a reactive df, but don't include ()
        var_to_plot = expr(TotalFreq_hour1), #as expr()
        var_to_plot_BW1 = expr(TotalFreq_hour1_BW1), #as expr()
        var_to_plot_BW2 = expr(TotalFreq_hour1_BW2), #as expr()
        treatment = selectData$treatment, #reactive value, but don't include ()
        grouping_var = selectData$grouping_var, #reactive value, but don't include ()
        expand_to_zero = reactive({ input$expand_to_zero }), #TRUE/FALSE
        dotLayer = reactive({ input$dot_plot }), #TRUE/FALSE
      ...
      InterEvent_plots <- burstParamsServer(
        "InterEvent",
        df = KNDyDATA_filtered, #use a reactive df, but don't include ()
        var_to_plot = expr(InterEvent_hour1), #as expr()
        var_to_plot_BW1 = expr(InterEvent_hour1_BW1), #as expr()
        var_to_plot_BW2 = expr(InterEvent_hour1_BW2), #as expr()
        treatment = selectData$treatment, #reactive value, but don't include ()
        grouping_var = selectData$grouping_var, #reactive value, but don't include ()
        expand_to_zero = reactive({ input$expand_to_zero }), #TRUE/FALSE
        dotLayer = reactive({ input$dot_plot }), #TRUE/FALSE
      ...
      intraBurst_plots <- burstParamsServer(
        "intraBurst",
        df = KNDyDATA_filtered, #use a reactive df, but don't include ()
        var_to_plot = expr(IntraBurst_hour1), #as expr()
        var_to_plot_BW1 = expr(IntraBurst_hour1_BW1), #as expr()
        var_to_plot_BW2 = expr(IntraBurst_hour1_BW2), #as expr()
        treatment = selectData$treatment, #reactive value, but don't include ()
        grouping_var = selectData$grouping_var, #reactive value, but don't include ()
        expand_to_zero = reactive({ input$expand_to_zero }), #TRUE/FALSE
        dotLayer = reactive({ input$dot_plot }), #TRUE/FALSE
