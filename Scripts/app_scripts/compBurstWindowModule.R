...
compBurstWindowUI <- function(id
  ...
  tagList(
    ...
    fluidRow(
      column(4,
             #expand to zero
             checkboxInput(inputId = ns("expand_to_zero"),
                           "Expand to zero",
                           value = TRUE)
             ),
      column(4,
             #dot plot layer
             checkboxInput(inputId = ns("dot_plot"),
                           label = "Add dot plot layer",
                           value = TRUE)
             ),
      column(4,
             checkboxInput(inputId = ns("violin_plot"),
    ...
    tabsetPanel(
      tabPanel("Bursts Per Hour",
               #have to render UI output because use the BW1 and BW2 values from above
               uiOutput(ns("burstsPerHour"))
               ),
      tabPanel("Mean Burst Duration",
               uiOutput(ns("mbd"))
               ),
      tabPanel("Spikes Per Burst",
               uiOutput(ns("spb"))
               ),
      tabPanel("Burst Frequency",
               uiOutput(ns("burstFreq"))
               ),
      tabPanel("Single Spike Number",
               uiOutput(ns("ssn"))
               ),
      tabPanel("Single Spike Frequency",
               uiOutput(ns("ssf"))
               ),
      tabPanel("Total Frequency",
               uiOutput(ns("totalFreq"))
               ),
      tabPanel("Interburst Interval",
               uiOutput(ns("interBurst"))
               ),
      tabPanel("Intraburst interval",
...
compBurstWindowServer <- function(id,
                              KNDyDATA,
                              KNDyDATA_adult,
                              KNDyDATA_juv
){
  moduleServer(
    ...
    function(input, output, session) {
      ...
      output$burstsPerHour <- renderUI({
        ns <- session$ns
        burstParamsUI(ns("burstsPerHour"),
                      bw1 = input$BW1,
                      bw2 = input$BW2,
      ...
      burstsPerHour_plots <- burstParamsServer("burstsPerHour",
      ...
      mbd_plots <- burstParamsServer("mbd",
                                       df = KNDyDATA_filtered, #use a reactive df, but don't include ()
      ...
      spb_plots <- burstParamsServer("spb",
                                     df = KNDyDATA_filtered, #use a reactive df, but don't include ()
      ...
      burstFreq_plots <- burstParamsServer("burstFreq",
                                     df = KNDyDATA_filtered, #use a reactive df, but don't include ()
      ...
      ssn_plots <- burstParamsServer("ssn",
                                     df = KNDyDATA_filtered, #use a reactive df, but don't include ()
      ...
      ssf_plots <- burstParamsServer("ssf",
                                     df = KNDyDATA_filtered, #use a reactive df, but don't include ()
      ...
      totalFreq_plots <- burstParamsServer("totalFreq",
                                     df = KNDyDATA_filtered, #use a reactive df, but don't include ()
      ...
      interBurst_plots <- burstParamsServer("interBurst",
                                     df = KNDyDATA_filtered, #use a reactive df, but don't include ()
      ...
      intraBurst_plots <- burstParamsServer("intraBurst",
                                     df = KNDyDATA_filtered, #use a reactive df, but don't include ()
