...
This removes cells excluded for reasons other than length of recording
```{r}
...
#### ANOVA
This is comparing only the juveniles by treatment group and slicing experimenter
```{r}
...
#### ANOVA
This is comparing only the juveniles by treatment group and recording experimenter
```{r}
KNDyDATA_juv_treatWhoRecorded_contrasts <- makeTreatandWhoRecordedContrasts(KNDyDATA_juv)
SpontAvgFiring_treatWhoRecorded_model <- lm_byTreatxWho(expr(SpontAvgFiring), KNDyDATA_juv_treatWhoRecorded_contrasts)
SpontAvgFiring_treatWhoRecorded_model <- makeANOVA_TreatWho(SpontAvgFiring_treatWhoRecorded_model) %>%
  kable_styling(
    font_size = 18,
    bootstrap_options = c("striped"), full_width = TRUE
  )
